...
class OpenAIProvider(LLMProvider):
    def __init__(self, settings: Settings):
        ...
        self._client = async_openai(api_key=settings.openai_api_key, timeout=30.0)
    ...
    async def reconcile_medications(
        ...
        data = await self._request_json_with_parse_retry(
            ...
            fallback=self._reconciliation_fallback(
                "Malformed or unavailable model response"
            ),
        )
    ...
    async def assess_data_quality(
        ...
        data = await self._request_json_with_parse_retry(
            ...
            user_prompt=user_prompt,
            fallback=self._data_quality_fallback(
                "Malformed or unavailable model response"
            ),
        )
        return DataQualityResponse.model_validate(data)
